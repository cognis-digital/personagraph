...
How **personagraph** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `personagraph` lives in the **Identity / persona** cluster.
...
| from | relation | to |
|---|---|---|
| `personagraph` | JSON findings compose with | [`corpmap`](https://github.com/cognis-digital/corpmap) |
| `personagraph` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `personagraph` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `personagraph` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the identity / persona cluster.** `personagraph` output feeds the next tool:
```bash
personagraph ... --format json > out.json          # this tool's findings (see `personagraph --help`)
...
personagraph ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
personagraph ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
personagraph ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `personagraph`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `personagraph` slots

"""Native cognis-connect emit for personagraph — forward findings to any platform.

Maps personagraph's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    personagraph ... --format json | personagraph-emit --to stix
    personagraph-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('identifier', {}).get('normalized', '')
        out['severity'] = 'medium'
        out['type'] = 'identity'
        out['description'] = 'Identity resolution dossier'
        out['tags'] = rec.get('platforms', [])
